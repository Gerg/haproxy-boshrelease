...
`
	...
	It("Correctly starts if there is a healthy backend", func() {
		...
		closeLocalServer, backendLocalPort, err := startLocalHTTPServer(func(w http.ResponseWriter, r *http.Request) {
			fmt.Fprintln(w, "Hello cloud foundry")
		})
		defer closeLocalServer()
		Expect(err).NotTo(HaveOccurred())

		By(fmt.Sprintf("Creating a reverse SSH tunnel from HAProxy backend (port %d) to local HTTP server (port %d)", haproxyBackendPort, backendLocalPort))
		ctx, cancelFunc := context.WithCancel(context.Background())
		defer cancelFunc()
		err = startReverseSSHPortForwarder(backendHaproxyInfo.SSHUser, backendHaproxyInfo.PublicIP, backendHaproxyInfo.SSHPrivateKey, haproxyBackendPort, backendLocalPort, ctx)
		Expect(err).NotTo(HaveOccurred())
